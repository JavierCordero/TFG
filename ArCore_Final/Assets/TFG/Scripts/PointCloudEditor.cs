...
public class PointCloudEditor : MonoBehaviour
{
	...
	private bool EnablePopAnimation = true;
	...
	[HideInInspector]
	public int actualPrefabToInstantiate = 0;
	...
	{
		for (int i = 0; i < m_MaxPointCount[actualMaxPointCount]; i++)
		{
			GameObject sphere = Instantiate(PrefabsToInstantiate[actualPrefabToInstantiate]);
	...
	private void _AddPointsIncrementallyToCache()
	{
		...
		{
			int iterations = Mathf.Min(MaxPointsToAddPerFrame[actualMaxPointPerFrameCount], Frame.PointCloud.PointCount);
	...
	{
		if (m_CachedPoints.Count >= m_MaxPointCount[actualMaxPointCount])
		{
